...
object Helper {
  def rmse(labelsAndPreds: RDD[(Double, Double)]): Double = {
    val rmsquarederror = labelsAndPreds.map(  x => math.pow(x._1 - x._2, 2)).mean()
    return math.sqrt(rmsquarederror)
  ...
  def predictOne(weights: Vector, features: Vector): Double = {
    ...
    //prediction = w⊺ ⋅ xj
    //val weight:Array[Double] = weights.toArray
    //val feature:Array[Double] = features.toArray
    //val prediction: Array[Double] = weight*feature // Product Not working
    return VectorHelper.dot(weights, features)

  }

  def predict(weights: Vector, data: RDD[Instance]): RDD[(Double, Double)] = {
    //  apply the predict formula below and in the end return an RDD of (label, prediction)
    return data.map( x => (x.label, Helper.predictOne(weights,x.features)))
...
class MyLinearRegressionImpl(override val uid: String)
  ...
  def gradientSummand(weights: Vector, lp: Instance): Vector = {
    ...
    val gdsmd:Double = VectorHelper.dot(weights, lp.features) - lp.label
    return VectorHelper.dot(lp.features, gdsmd)
  }

  def gradient(d: RDD[Instance], weights: Vector): Vector = {

    val grad = d.map( x => gradientSummand( weights, x ))
